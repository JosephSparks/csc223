...
int main() {
    ...
    for (int i = 0; i < length; i++) {
        ...
        for (int j = 0; j < info->length; j++) {
            int select_index = info->length - j - 1;
            int bit = (info->code >> select_index) & 1;
            encode_buf[total_bit_length / 8] |= bit << (7 - (total_bit_length % 8));
            total_bit_length++;
        }
    }

    // print out encoded data in hex
    FILE* encoded_file;
    encoded_file = fopen("alice-encoded.bin", "w");
    fwrite(encode_buf, 1, total_bit_length / 8 + 1, encoded_file);
    fclose(encoded_file);
}

void decode_file(char* encoded_file_path, TreeNode* tree) {
    // Read the encoded file
    int encoded_length;
    char* encoded_data = read_file(encoded_file_path, &encoded_length);
    ...
    for (int i = 0; i < encoded_length * 8; i++) {
        // Traverse the tree based on the bit read from the encoded data
        ...
        // If a leaf node is reached, write the corresponding symbol to the decoded file
        if (current_node->left == NULL && current_node->right == NULL) {
            fprintf(decoded_file, "%c", current_node->data);
